...
function MealDetails({meal,D}) {
  ...
  return (
    <div style={{marginTop:12,paddingTop:12,borderTop:"1px solid rgba(255,255,255,0.4)"}}>
      {meal.goal_alignment&&<div style={{marginBottom:8,fontSize:D.md}}><span style={{color:G.hint,fontSize:D.sm}}>Alineación con objetivo — </span><span style={{color:alignC[meal.goal_alignment]||G.muted,fontWeight:600}}>{meal.goal_alignment}</span></div>}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          ...
          <div>
            ...
            <p style={{margin:"3px 0 0",fontSize:D.sm,color:G.hint}}>IMC {bmi}{profile.age?` · ${profile.age} años`:""}</p>
          </div>
        </div>
      ...
      <div style={{...glassCard,padding:28,width:"100%",maxWidth:440,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"flex",gap:12,marginBottom:20}}>
          <div style={{background:G.sageLight,borderRadius:10,padding:"10px 14px",flex:1,textAlign:"center"}}>
            ...
            <p style={{margin:0,fontSize:24,fontWeight:600,color:G.sage}}>{avgScore}</p>
          </div>
          <div style={{background:G.sageLight,borderRadius:10,padding:"10px 14px",flex:1,textAlign:"center"}}>
            <p style={{margin:"0 0 2px",fontSize:11,color:G.hint,letterSpacing:"0.04em"}}>PROTEÍNA</p>
            <p style={{margin:0,fontSize:24,fontWeight:600,color:G.sage}}>{totProt}g</p>
          </div>
          <div style={{background:G.sageLight,borderRadius:10,padding:"10px 14px",flex:1,textAlign:"center"}}>
            <p style={{margin:"0 0 2px",fontSize:11,color:G.hint,letterSpacing:"0.04em"}}>COMIDAS</p>
          ...
          <div>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
              <div style={{textAlign:"center"}}>
                <p style={{margin:0,fontSize:40,fontWeight:300,color:scoreColor,lineHeight:1}}>{analysis.puntuacion_dia}</p>
                <p style={{margin:"2px 0 0",fontSize:11,color:G.hint}}>/10</p>
              </div>
              <p style={{margin:0,fontSize:D.md,color:G.muted,lineHeight:1.6,flex:1}}>{analysis.resumen}</p>
            </div>
            {(analysis.logros||[]).length>0&&<div style={{marginBottom:12}}>{analysis.logros.map((l,i)=><div key={i} style={{fontSize:D.sm,color:G.sage,marginBottom:4}}>✓ {l}</div>)}</div>}
            {(analysis.areas_mejora||[]).length>0&&<div style={{marginBottom:12}}>{analysis.areas_mejora.map((a,i)=><div key={i} style={{fontSize:D.sm,color:G.gold,marginBottom:4}}>→ {a}</div>)}</div>}
    ...
    <div>
      <div style={{...glassCard,padding:"18px 20px",marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          {missingSlots.length===0?<span style={{fontSize:13,color:G.sage,fontWeight:500}}>✓ Día completo</span>:<span style={{fontSize:13,color:G.hint}}>{missingSlots.length} comida{missingSlots.length!==1?"s":""} pendiente{missingSlots.length!==1?"s":""}</span>}
        </div>
      ...
      <div style={{maxWidth:isDesktop?720:640,margin:"0 auto",padding:isDesktop?"28px 40px":"20px 20px"}}>
        ...
        <div style={{...glassCard,padding:"14px 20px",marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            ...
            <span style={{color:protColor,fontWeight:600,fontSize:D.sm}}>{Math.round(todayProt)}g / <span style={{fontSize:"1.1em",fontWeight:700}}>{proteinGoal}g</span></span>
          </div>
          ...
          {profile?.tdee&&(
            <>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                ...
                <span style={{color:calColor,fontWeight:600,fontSize:D.sm}}>{Math.round(todayCal)} / <span style={{fontSize:"1.1em",fontWeight:700}}>{profile.tdee.toLocaleString()} kcal</span></span>
              </div>
        ...
        {tab==="today"&&(
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,margin:"8px 0 16px"}}>
              <div style={{flex:1,height:1,background:"rgba(255,255,255,0.4)"}}/>
              <span style={{fontSize:D.sm,color:G.hint,letterSpacing:"0.04em"}}>FIN DEL DÍA</span>
              <div style={{flex:1,height:1,background:"rgba(255,255,255,0.4)"}}/>
            </div>
