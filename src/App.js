...
// ID de usuario fijo por ahora (sin login)
const USER_ID = "gabo";
...
const G = {
  ...
  sage:         "#5a7a54",
  sageMid:      "#87a882",
  sageLight:    "rgba(138,180,132,0.18)",
  sageBorder:   "rgba(90,122,84,0.3)",
  gold:         "#8a6c2a",
  goldLight:    "rgba(180,148,72,0.15)",
  red:          "#8a4040",
  redLight:     "rgba(180,80,80,0.12)",
  bg1:          "#c8d8c4",
  bg2:          "#e8e0d0",
  bg3:          "#d4cabb",
};
...
const LEVELS  = [
  ...
  { name:"Aprendiz",   emoji:"🌿", xp:200 },
  { name:"Consciente", emoji:"🍃", xp:500 },
  { name:"Atleta",     emoji:"🌾", xp:1000 },
  { name:"Leyenda",    emoji:"🌳", xp:2000 },
];
const BADGES_DEF = [
...
async function saveDay(fecha, dayData) {
  await supabase.from("registros").upsert({
    ...
    workout: dayData.workout || null,
  }, { onConflict: "user_id,fecha" });
}

// localStorage para XP, badges y streak (datos de gamificación)
...
function Confetti({ active }) {
  ...
  const pieces = Array.from({length:22},(_,i)=>({
    ...
    color:[G.sage,G.sageMid,G.gold,"#c8b89a","#8fad8b"][i%5],
    delay:Math.random()*0.4, size:5+Math.random()*6,
  }));
  return (
    <div style={{position:"fixed",top:0,left:0,width:"100%",height:"100%",pointerEvents:"none",zIndex:9999}}>
      {pieces.map(p=>(
        <div key={p.id} style={{
          ...
          animation:`fall 1.6s ${p.delay}s ease-in forwards`,
        }}/>
      ))}
...
function Toast({ toasts }) {
  return (
    <div style={{position:"fixed",bottom:24,right:20,zIndex:9998,display:"flex",flexDirection:"column",gap:8}}>
      {toasts.map(t=>(
        <div key={t.id} style={{
          background:G.glass,backdropFilter:blur,WebkitBackdropFilter:blur,
          border:`1px solid ${G.border}`,color:G.text,
          padding:"12px 16px",borderRadius:14,
          display:"flex",alignItems:"center",gap:10,
          borderLeft:`3px solid ${G.sage}`,animation:"slideIn 0.3s ease",
        }}>
          <span style={{fontSize:20}}>{t.emoji}</span>
          <div>
            <div style={{fontWeight:600,fontSize:13}}>{t.title}</div>
            <div style={{fontSize:11,color:G.muted}}>{t.desc}</div>
          </div>
        </div>
      ))}
...
const glassCard = {
  background:G.glass, backdropFilter:blur, WebkitBackdropFilter:blur,
  border:`1px solid ${G.border}`, borderRadius:18,
};
const glassSubtle = {
  background:G.glassDark, backdropFilter:blurSm, WebkitBackdropFilter:blurSm,
  border:`1px solid ${G.borderSubtle}`, borderRadius:12,
};
const inp = {
  width:"100%", background:"rgba(255,255,255,0.5)", backdropFilter:blurSm,
  WebkitBackdropFilter:blurSm, border:`1px solid ${G.border}`,
  borderRadius:10, color:G.text, padding:"10px 13px", fontSize:14,
  boxSizing:"border-box", marginBottom:8, outline:"none", fontFamily:"inherit",
};
...
function Btn({ onClick, loading, children, full, disabled }) {
  return (
    <button onClick={onClick} disabled={loading||disabled} style={{
      ...
      border:`1px solid ${loading||disabled?G.borderSubtle:G.sageBorder}`,
      cursor:"pointer",
      background:loading||disabled?"rgba(255,255,255,0.3)":"rgba(90,122,84,0.15)",
      backdropFilter:blurSm, WebkitBackdropFilter:blurSm,
      color:loading||disabled?G.hint:G.sage,
...
function Tag({ color, bg, border, children }) {
  return (
    <span style={{
      fontSize:11, background:bg||G.sageLight, color:color||G.sage,
      border:`1px solid ${border||G.sageBorder}`,
      padding:"3px 9px", borderRadius:99, fontWeight:500, letterSpacing:"0.02em",
    }}>{children}</span>
  );
}

function Divider() {
  return <div style={{height:"1px",background:"rgba(255,255,255,0.4)",margin:"0 18px"}}/>;
}
...
function XPBar({ xp, streak }) {
  ...
  return (
    <div style={{...glassCard,padding:"14px 18px",marginBottom:16,display:"flex",alignItems:"center",gap:14}}>
      <span style={{fontSize:24}}>{lvl.emoji}</span>
      <div style={{flex:1}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:6}}>
          <span style={{fontWeight:600,color:G.sage,letterSpacing:"0.04em"}}>{lvl.name}</span>
          <span style={{color:G.hint,fontSize:11}}>{next?`${xp} / ${next.xp} xp`:"Máximo"}</span>
        </div>
        <div style={{background:"rgba(255,255,255,0.35)",borderRadius:99,height:5,overflow:"hidden"}}>
          <div style={{width:`${pct}%`,background:G.sage,height:"100%",borderRadius:99,transition:"width 0.6s ease",opacity:0.8}}/>
        </div>
      </div>
      {streak>0&&(
        <div style={{...glassSubtle,display:"flex",flexDirection:"column",alignItems:"center",padding:"6px 10px",minWidth:38,borderRadius:10}}>
          <span style={{fontSize:14}}>🔥</span>
          <span style={{fontSize:10,color:G.gold,fontWeight:600,marginTop:1}}>{streak}d</span>
        </div>
      )}
    </div>
  );
...
function MealDetails({ meal }) {
  ...
  return (
    <div style={{marginTop:12,paddingTop:12,borderTop:"1px solid rgba(255,255,255,0.4)",fontSize:13}}>
      <div style={{marginBottom:8}}>
        <span style={{color:G.hint,fontSize:12}}>Hipertrofia — </span>
        <span style={{color:hyC[meal.hypertrophy]||G.muted,fontWeight:600}}>{meal.hypertrophy}</span>
      </div>
      {(meal.nutrients||[]).map((n,i)=>(
        <div key={i} style={{display:"flex",gap:8,marginBottom:5}}>
          <span style={{color:G.sage,fontWeight:600,minWidth:80,fontSize:12}}>{n.name}</span>
          <span style={{color:G.muted,fontSize:12,lineHeight:1.4}}>{n.benefit}</span>
        </div>
      ))}
      {meal.tip&&(
        <div style={{marginTop:10,background:G.sageLight,border:`1px solid ${G.sageBorder}`,borderRadius:8,padding:"10px 12px",color:G.sage,fontSize:12,fontStyle:"italic"}}>
          💡 {meal.tip}
        </div>
      )}
    </div>
        ...
        : (
          <div style={{display:"flex",gap:8}}>
            <input value={input} onChange={e=>onInput(e.target.value)}
              placeholder="¿Qué comiste?" style={{...inp,flex:1,marginBottom:0}}
              onKeyDown={e=>e.key==="Enter"&&onSubmit()}/>
            <Btn loading={loading} onClick={onSubmit}>Analizar</Btn>
          </div>
        )
      }
    </div>
...
function WorkoutCard({ workout, compact }) {
  return (
    <div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:compact?0:10}}>
        <Tag>{workout.type}</Tag>
        <Tag>{workout.duration} min</Tag>
        <Tag>{"●".repeat(workout.intensity)} int.</Tag>
      </div>
      {!compact&&workout.coherence_score&&(
        <div style={{marginTop:12}}>
          <div style={{fontSize:13,marginBottom:8}}>
            <span style={{color:G.hint,fontSize:12}}>Coherencia — </span>
            <span style={{fontWeight:600,color:G.sage}}>{workout.coherence_score}/10</span>
          </div>
          <p style={{fontSize:12,color:G.muted,margin:"0 0 8px",lineHeight:1.5}}>{workout.balance}</p>
          {(workout.strengths||[]).map((s,i)=>(
            <div key={i} style={{fontSize:12,color:G.sage,marginBottom:3}}>✓ {s}</div>
          ))}
          {(workout.suggestions||[]).map((s,i)=>(
            <div key={i} style={{fontSize:12,color:G.gold,marginBottom:3}}>→ {s}</div>
          ))}
        </div>
      )}
    </div>
...
function WeekRanking({ days }) {
  ...
  const ranked=Object.entries(weeks).map(([wk,arr])=>({
    wk, score:(arr.reduce((a,b)=>a+b,0)/arr.length).toFixed(1), days:arr.length,
  })).sort((a,b)=>b.score-a.score);
  return (
    <div style={{...glassSubtle,padding:16,marginTop:10,borderRadius:14}}>
      <p style={{margin:"0 0 10px",fontSize:13,fontWeight:600,color:G.text}}>Mejores semanas</p>
      {ranked.slice(0,5).map((r,i)=>(
        <div key={r.wk} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"7px 0",borderBottom:"1px solid rgba(255,255,255,0.3)"}}>
          <span style={{color:G.muted}}>{["🥇","🥈","🥉","4","5"][i]} {r.wk}</span>
          <span style={{color:G.sage,fontWeight:600}}>⭐ {r.score} · {r.days}d</span>
        </div>
      ))}
    </div>
  );
    ...
    <div style={{...glassCard,padding:"16px 18px",marginBottom:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        ...
        <div>
          ...
          <input type="number" value={form.height} onChange={e=>setForm(f=>({...f,height:e.target.value}))} style={inp}/>
        </div>
      </div>
      {form.weight&&form.height&&(
        <div style={{background:G.sageLight,border:`1px solid ${G.sageBorder}`,borderRadius:8,padding:"8px 12px",fontSize:12,color:G.sage,marginBottom:10}}>
          Meta proteína: <strong>{Math.round(+form.weight*2)}g/día</strong> · IMC: {(+form.weight/((+form.height/100)**2)).toFixed(1)}
        </div>
      )}
      <div style={{display:"flex",gap:8}}>
  ...
  return (
    <div style={{...glassCard,padding:"14px 18px",marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div style={{display:"flex",gap:14,alignItems:"center"}}>
        <div style={{width:36,height:36,borderRadius:"50%",background:G.sageLight,border:`1px solid ${G.sageBorder}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15}}>
          🌿
        </div>
        <div>
          <p style={{margin:0,fontSize:13,fontWeight:600,color:G.text}}>{profile.name}</p>
          <p style={{margin:"2px 0 0",fontSize:11,color:G.hint}}>{profile.weight}kg · {profile.height}cm · IMC {bmi}</p>
        </div>
      </div>
    ...
    <div style={{
      ...
      <div style={{...glassCard,padding:32,maxWidth:360,width:"100%"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <div>
            <label style={lbl}>Peso (kg)</label>
            <input type="number" min="30" max="200" value={form.weight}
              onChange={e=>setForm(f=>({...f,weight:e.target.value}))} placeholder="70" style={inp}/>
          </div>
          <div>
            <label style={lbl}>Altura (cm)</label>
            <input type="number" min="100" max="230" value={form.height}
              onChange={e=>setForm(f=>({...f,height:e.target.value}))} placeholder="175" style={inp}/>
          </div>
        </div>
        {prot&&(
          <div style={{background:G.sageLight,border:`1px solid ${G.sageBorder}`,borderRadius:10,padding:"12px 14px",marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:G.sage}}>
              <div>
                <p style={{margin:0,color:G.hint,fontSize:11,letterSpacing:"0.04em"}}>META DE PROTEÍNA</p>
                <p style={{margin:"3px 0 0",fontSize:20,fontWeight:600,color:G.sage}}>{prot}g<span style={{fontSize:11,fontWeight:400,color:G.hint}}>/día</span></p>
              </div>
              {bmi&&(
                <div style={{textAlign:"right"}}>
                  <p style={{margin:0,color:G.hint,fontSize:11,letterSpacing:"0.04em"}}>IMC</p>
                  <p style={{margin:"3px 0 0",fontSize:20,fontWeight:600,color:G.sage}}>{bmi}</p>
                </div>
              )}
            </div>
...
export default function App() {
  ...
  const fetchWeekSummary = async () => {
    ...
    const data7 = last7.map(d=>`${d.date}: score ${d.score}, prot ${d.prot}g`).join("; ");
    try {
  ...
  const last7 = Array.from({length:7},(_,i)=>{
    ...
    return {date:k,label:d.toLocaleDateString("es",{weekday:"short"}),score:Math.round(score*10)/10,prot:Math.round(prot)};
  });

  const weekWorkoutCount = Object.entries(days).filter(([d,v])=>{
    const diff=(new Date(today)-new Date(d))/86400000;
    return diff>=0&&diff<7&&v?.workout;
  }).length;
  ...
  return (
    <div style={{
      ...
      color:G.text, padding:20, maxWidth:600, margin:"0 auto",
    }}>
      ...
      <div style={{...glassCard,padding:"12px 18px",marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:8}}>
          ...
          <span style={{color:todayProt>=proteinGoal?G.sage:G.gold,fontWeight:600}}>
            {Math.round(todayProt)}g / {proteinGoal}g
          </span>
        </div>
      ...
      <div style={{...glassSubtle,display:"flex",gap:0,marginBottom:20,overflow:"hidden",padding:4,borderRadius:14}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{
            flex:1,padding:"8px 6px",background:tab===t.id?"rgba(255,255,255,0.6)":"transparent",
            border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:12,
            fontWeight:tab===t.id?600:400,color:tab===t.id?G.sage:G.hint,
            borderRadius:10,transition:"all 0.2s",
            backdropFilter:tab===t.id?blurSm:"none",WebkitBackdropFilter:tab===t.id?blurSm:"none",
          }}>{t.label}</button>
        ))}
      </div>
        ...
        <div>
          <div style={{...glassCard,overflow:"hidden",marginBottom:16}}>
            {FIXED_SLOTS.map((slot,i)=>(
              <div key={slot.id}>
                <MealSlot slot={slot} meal={todayData.meals?.[slot.id]}
                  input={mealInputs[slot.id]||""} onInput={v=>setMealInputs(x=>({...x,[slot.id]:v}))}
                  loading={mealLoading[slot.id]} onSubmit={()=>handleMealSubmit(slot.id,slot.label)}/>
          ...
          <div style={{...glassCard,padding:"16px 18px"}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <input value={snackName} onChange={e=>setSnackName(e.target.value)}
                placeholder="Nombre del tentempié" style={{...inp,marginBottom:0,flex:1}}/>
            </div>
            <div style={{display:"flex",gap:8}}>
              <input value={mealInputs["snack_new"]||""} onChange={e=>setMealInputs(x=>({...x,snack_new:e.target.value}))}
                placeholder="Describí qué comiste…" style={{...inp,flex:1,marginBottom:0}}
                onKeyDown={e=>e.key==="Enter"&&handleSnackSubmit()}/>
        ...
        <div style={{...glassCard,padding:20}}>
          ...
          {todayData.workout ? <WorkoutCard workout={todayData.workout}/> : (
            <div>
              ...
              <input type="range" min={15} max={180} step={5} value={workoutForm.duration}
                onChange={e=>setWorkoutForm(f=>({...f,duration:+e.target.value}))} style={{width:"100%",marginBottom:8}}/>
              <label style={lbl}>Intensidad — {workoutForm.intensity} / 5</label>
              <input type="range" min={1} max={5} step={1} value={workoutForm.intensity}
                onChange={e=>setWorkoutForm(f=>({...f,intensity:+e.target.value}))} style={{width:"100%",marginBottom:8}}/>
              <label style={lbl}>Notas</label>
              <textarea value={workoutForm.notes} onChange={e=>setWorkoutForm(f=>({...f,notes:e.target.value}))}
                placeholder="Opcional…" rows={2} style={{...inp,resize:"vertical"}}/>
              ...
              <div>
                <div style={{...glassCard,padding:"14px 18px",marginBottom:12}}>
                  <p style={{margin:"0 0 8px",fontWeight:500,fontSize:14}}>
                    {new Date(histDate+"T12:00:00").toLocaleDateString("es",{weekday:"long",day:"numeric",month:"long"})}
                  </p>
                ...
                {FIXED_SLOTS.map(slot=>{
                  ...
                  return (
                    <div key={key} style={{...glassCard,padding:"14px 18px",marginBottom:8}}>
                      <div style={{display:"flex",justifyContent:"space-between",cursor:"pointer",marginBottom:6}}
                        onClick={()=>setExpandedMeal(isExp?null:key)}>
